const getStatusStyle = (label:string) => {
  ...
  const StatusTextBadge = ({ status }:{status:{label:string}}) => {
    const { bgColor, textColor, isBordered } = getStatusStyle(status?.label);
    const borderClass = isBordered ? "border" : "";
  
    return (
      <span
        className={`${borderClass} font-semibold text-[8px]   text-center whitespace-nowrap block`}
        style={{ color: bgColor}}
      >
        {status?.label || "N/A"}
      </span>
  ...
  export default StatusTextBadge
