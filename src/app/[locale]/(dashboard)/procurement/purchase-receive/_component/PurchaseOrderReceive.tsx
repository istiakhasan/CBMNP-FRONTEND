...
const PurchaseOrderReceive = ({ rowData, setReceiveModal,setRowData }: any) => {
          ...
          <table className="w-full border border-gray-200 text-left text-sm">
            ...
            <tbody>
              {rowData?.items?.map((item: any, i: any) => (
                <tr className={`${item?.orderedQuantity===item?.receivedQuantity?"bg-[#4F8A6D80]":""}`} key={i}>
                  <td className="p-2 border">{i+1}</td>
                  <td className="p-2 border">{item?.product?.name}</td>
                  <td className="p-2 border">{moment(item?.createdAt).format('YYYY-MM-DD')}</td>
                  <td className="p-2 border">
                    <input
                      type="number"
                      className="w-full p-1 border rounded"
                      placeholder="Enter qty"
                      disabled={item?.orderedQuantity===item?.receivedQuantity}
                      value={item?.quantityToReceive || ''}
                      onChange={(e) => {
                        console.log(+e.target.value ,+item?.orderedQuantity);
                        if(+e.target.value>+item?.orderedQuantity){
                            return message.error('Receive quantity is not greater then order quantity')
                        }
                        const _data=[...rowData.items]
                        _data[i]={..._data[i],quantityToReceive:e.target.value}
        ...
        <div className="w-[300px] border-l border-gray-200 pl-3">
          ...
          <div className="text-sm space-y-1">
            <p className="flex justify-between">
              <span>Net Total:</span> <span>{rowData?.items?.reduce((a:any,b:any)=>a+(b.unitPrice*(b.quantityToReceive || 0)),0)} (tk)</span>
            </p>
            <p className="flex justify-between">
            ...
            <p className="flex justify-between font-semibold">
              <span>Purchase Total:</span> <span>{rowData?.items?.reduce((a:any,b:any)=>a+(b.unitPrice*(b.quantityToReceive || 0)),0)} (tk)</span>
            </p>
            <p className="flex justify-between">
         ...
         onClick={async()=>{
          try {
            if(!watch()?.warehouse && !watch()?.warehouse?.value){
              ...
              const modifiedData=rowData?.items?.map((item:any)=>{
                 return {
              ...
              const poIds=rowData?.items?.filter((item:any)=>+item?.quantityToReceive>0) .map((item:any)=>{
                 return {
                  productId:item?.productId,
                  id:item?.id,
                  receivedQuantity:+item?.receivedQuantity + +item?.quantityToReceive
                 } 
              })
             
  
              const result=await receivePurchaseOrder({stock:modifiedData,poIds}).unwrap()
              if(result){
