    ...
    <div>
      <div className="table_wrapper custom_scroll">
        <div className="bg-[#eeeeee] h-[30px] sticky bottom-0 flex  px-3 justify-between items-center mb-1 gap-[10px]">
          {reports?.data?.length>0 && <div>
            <strong>{moment(reports?.meta?.startDate).format('DD-MM-YYYY')} To {moment(reports?.meta?.endDate).format('DD-MM-YYYY')}</strong>
          </div>}
          <div>
          ...
          <tbody>
            {reports?.data?.map((row: any, i: number) => (
              <tr key={i}>
                ...
                <td style={{textAlign:"center"}}>{row?.totalOrderQuantity || "N/A"}</td>
                <td style={{textAlign:"end"}}>{row?.totalSaleAmount || "N/A"}</td>
            ...
            <tr>
                ...
                <th>
                    <span  className="flex justify-center gap-2 font-bold">
                        <span>Total:</span>
                        <span>{reports?.data?.reduce((a:any,b:any)=>a+b?.totalOrderQuantity,0) || 0}</span>
                    </span>
                </th>
                <th>
                    <span  className="flex justify-end gap-2 font-bold">
                        <span>Total Sales:</span>
                        <span>{reports?.data?.reduce((a:any,b:any)=>a+b?.totalSaleAmount,0) || 0}</span>
                    </span>
                </th>
                <th>
                    <span  className="flex justify-end gap-2 font-bold">
                        <span>Total Orders:</span>
                        <span>{reports?.data?.reduce((a:any,b:any)=>a+b?.orderCount,0) || 0}</span>
                    </span>
                </th>
            </tr>
          </tfoot>
