...
export default function OrderSummary({
  ...
  if (orderDetails?.paymentMethod?.length === 0)
    missingRequirements.push("Select payment method(s)");
  ...
  if (
    (orderDetails.paymentStatus === "Partial" ||
      orderDetails.paymentStatus === "Paid") &&
    orderDetails?.transactionId?.length < 1
  )
    missingRequirements.push("TransectinId");
  if (
    (orderDetails.paymentStatus === "Partial" ||
      orderDetails.paymentStatus === "Paid") &&
    orderDetails?.amount < 1
...
console.log("orderSource exists:", orderDetails?.orderSource?.length > 0);
  return (
    <div className="lg:sticky lg:top-4 space-y-4 ">
            ...
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {cartItems.map((item) => {
                ...
                return (
                  <div
                    key={item.product.id}
                    className="space-y-3 p-4 bg-gradient-to-r  rounded-xl border-2  transition-all duration-200"
                  >
      ...
      <div className="lg:sticky fixed bottom-0 lg:bg-white left-0 right-0  lg:bottom-0 bg-white lg:bg-transparent p-4 lg:p-0 border-t lg:border-t-0 border-gray-200 lg:border-none shadow-lg lg:shadow-none z-50">
