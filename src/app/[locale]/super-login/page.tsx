...
import { useSignInMutation, useSuperAdminLogInMutation } from "@/redux/api/authApi";
...
const Page = () => {
  ...
  const [handleUserLogin] = useSuperAdminLogInMutation();
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      if (res?.data?.accessToken) {
        ...
        router.push(`/${local}/super-admin/dashboard`);
      }
    } catch (error: any) {
      if (error) {
  ...
  useEffect(() => {
    if (userInfo?.role) {
      return redirect(`/${local}/super-admin/dashboard`);
    }else{
      setLoading(false)
    }
  }, [userInfo?.role,local]);
